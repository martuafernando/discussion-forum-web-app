  ...
  async fetch(endpoint, method, body) {
    ...
    if (!response.ok) {
      throw Error(
          ...
          'There is problem when communicate to the server',
      );
    }
    return responseBody;
  }
...
const User = {
  getProfile: async () => await request.get('/users/me'),
  getUsers: async () => await request.get('/users'),
};

const Leaderboard = {
  getLeaderboard: async () => await request.get('/leaderboards'),
};
