...
import {
  ...
  asyncUpVoteComment,
} from '../../redux/states/thread/action';
  ...
  return (
    <ModalLayout>
      <div className="detail-thread-page">
            ...
            {thread.comments?.map((comment) => {
              return (
                <CommentItem
                  ...
                  comment={comment}
                  onUpVote={() =>
                    dispatch(asyncUpVoteComment(thread.id, comment.id, user.id))
                  }
                  onDownVote={() =>
                    dispatch(
                        asyncDownVoteComment(thread.id, comment.id, user.id),
                    )
                  }
                  onCancelVote={() =>
                    dispatch(
                        asyncNeutralVoteComment(thread.id, comment.id, user.id),
                    )
                  }
                />
              );
        ...
        <form
          className="detail-thread-page__add-comment-form"
          onSubmit={onSubmitHandler}
        >
          <Flash />
          <div className="detail-thread-page__add-comment">
            <img className="item-author__avatar" src={user.avatar} />
            <div
              ...
              contentEditable
            />
            <button className="detail-thread-page__send-comment" type="submit">
              <FaPaperPlane />
            </button>
          </div>
        </form>
