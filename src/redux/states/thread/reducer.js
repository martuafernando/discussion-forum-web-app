  ...
  switch (action.type) {
    ...
    case INCREASE_COMMENT_LIKES: {
      if (action.payload.threadId === state.id) {
        ...
        const updatedComment = {
          ...
          upVotesBy: [
            ...state.comments[commentIndex].upVotesBy,
            action.payload.userId,
          ],
          downVotesBy: state.comments[commentIndex].downVotesBy.filter(
              (it) => it !== action.payload.userId,
          ),
        };
        const updatedThread = {
    ...
    case INCREASE_COMMENT_DISLIKES: {
      if (action.payload.threadId === state.id) {
        ...
        const updatedComment = {
          ...state.comments[commentIndex],
          upVotesBy: state.comments[commentIndex].upVotesBy.filter(
              (it) => it !== action.payload.userId,
          ),
          downVotesBy: [
            ...state.comments[commentIndex].downVotesBy,
            action.payload.userId,
          ],
        };
        const updatedThread = {
